...
1. 学生维护个人求职画像和简历。
2. 学生导入或粘贴目标岗位 JD，或授权系统从岗位库、学校就业源、第三方授权 API 中自动发现候选岗位。
3. 系统解析简历与岗位要求。
4. 系统生成岗位匹配评分、匹配解释和能力差距分析。
5. 系统生成分层岗位推荐组合，展示岗位匹配度、申请成功率预测、机会价值和风险等级，帮助学生平衡岗位质量与初筛通过概率。
6. 系统给出针对目标岗位的简历优化建议。
7. 学生接受建议后，可由 AI 在用户确认范围内生成新的简历版本。
8. 学生保存匹配报告和简历优化版本，用于后续投递决策。

微信小程序、手机 App、外部招聘平台 API、公开网页采集、自动投递等能力作为后续阶段扩展。岗位自动获取仅允许接入用户授权、学校授权或平台授权的数据源，MVP 不设计绕过招聘平台规则的采集能力。
...
| P06 | 投递组合缺少策略 | 学生只看单个岗位匹配分，缺少分层岗位推荐组合和申请成功率预测 | 容易只关注单一高分岗位或盲目扩大投递，投递节奏不稳定 |
...
2. 用 AI 自动发现候选岗位，并构建分层岗位推荐组合。
3. 用结构化评分解释简历与岗位之间的匹配关系，让学生知道强项与短板。
4. 用面向具体 JD 的简历优化建议和 AI 改写能力，提高简历初筛命中率。
5. 用历史报告和版本记录沉淀求职过程，帮助学生持续改进。
...
| G06 | 建立投递组合策略 | 推荐列表按机会梯度分层展示，并说明岗位匹配度、申请成功率预测、机会价值、风险等级和分层原因 |
| G07 | 缩短简历修改链路 | 用户采纳建议后可生成 AI 改写草稿，并以新简历版本保存 |
...
| 参与者 | 职责 |
| --- | --- |
| 学生用户 | 维护个人画像、上传简历、导入岗位、查看匹配报告、采纳优化建议并确认 AI 改写结果 |
| 系统智能体 | 解析简历和 JD，自动发现授权岗位，进行匹配评分，生成分层岗位推荐组合、解释和建议 |
| 岗位数据提供方 | 提供岗位描述，可来自学生粘贴、文件导入、后台维护、学校就业源或后续第三方授权 API |
...
| 岗位发现任务 | 系统基于用户画像自动获取候选岗位的任务 | 数据源、检索条件、任务状态、候选岗位数量、授权状态 |
| JD 解析结果 | 岗位要求的结构化表达 | 硬技能、软技能、经验要求、学历要求、关键词、加分项 |
| 匹配任务 | 一次简历与岗位的分析任务 | 简历版本、岗位、任务状态、模型版本、创建时间 |
| 匹配报告 | 匹配任务的分析输出 | 总分、维度分、优势、差距、证据、风险、建议 |
| 推荐组合 | 面向一次求职目标的岗位分层结果 | 拓展机会层、重点匹配层、基础保障层、岗位匹配度、申请成功率预测、机会价值、风险等级、分层理由、投递优先级 |
| 优化建议 | 针对目标岗位的简历修改建议 | 建议类型、优先级、原始问题、修改方向、示例表达 |
| AI 改写任务 | 用户接受建议后生成简历草稿的任务 | 建议范围、改写前文本、改写后文本、差异、用户确认状态 |
...
erDiagram
    ...
    STUDENT ||--o{ JOB_DISCOVERY_TASK : starts
    JOB_DISCOVERY_TASK ||--o{ JOB : discovers
    JOB ||--|| JD_ANALYSIS : has
    RESUME_VERSION ||--o{ MATCH_TASK : used_by
    JOB ||--o{ MATCH_TASK : compared_with
    MATCH_TASK ||--|| MATCH_REPORT : produces
    MATCH_REPORT ||--|| RECOMMENDATION_TIER : classifies
    MATCH_REPORT ||--o{ OPTIMIZATION_SUGGESTION : includes
    OPTIMIZATION_SUGGESTION ||--o{ RESUME_REWRITE_TASK : triggers
    RESUME_REWRITE_TASK ||--o{ RESUME_VERSION : creates
...
| BR08 | AI 自动获取岗位必须基于授权数据源或用户主动提供的数据，不得绕过招聘平台访问限制。 |
| BR09 | 分层岗位推荐必须同时考虑岗位匹配度、硬性门槛、申请成功率预测、机会价值、竞争风险和用户偏好，不能只按总分排序。 |
| BR10 | AI 改写简历只能在用户选择的建议范围内生成草稿，必须展示差异并经用户确认后才能保存为新版本。 |
...
| FR17 | 岗位发现 | AI 自动发现候选岗位 | P1 | 系统基于求职画像、简历技能和授权岗位源自动检索候选岗位，并过滤明显不符合硬性条件的岗位 |
| FR18 | 智能匹配 | 分层岗位推荐组合 | P1 | 将候选岗位划分为拓展机会、重点匹配、基础保障等机会梯度，展示岗位匹配度、申请成功率预测、分层理由、投递建议和风险提示 |
| FR19 | 简历优化 | AI 生成改写草稿 | P1 | 用户采纳建议后，系统基于原简历和选中建议生成可对比的改写草稿，用户确认后创建新简历版本 |
...
#### UC05 自动发现岗位并生成分层推荐组合

| 项目 | 描述 |
| --- | --- |
| 参与者 | 学生用户、系统智能体、岗位数据提供方 |
| 前置条件 | 学生已维护求职画像，系统存在后台岗位库或已授权岗位数据源 |
| 主流程 | 学生选择目标方向和数据源；系统检索候选岗位；解析 JD；过滤硬性条件明显不符的岗位；生成岗位匹配度、申请成功率预测、机会价值和风险等级；按机会梯度展示分层岗位推荐组合 |
| 后置条件 | 系统保存岗位发现任务、候选岗位、分层推荐结果和推荐理由 |
| 异常流程 | 数据源未授权、岗位信息不足或岗位已过期时，系统标记原因并提示用户使用粘贴 JD 方式补充 |
...
| --- | --- |
| 参与者 | 学生用户、系统智能体 |
| 前置条件 | 匹配报告已生成，且用户选择了一条或多条可改写建议 |
| 主流程 | 学生选择要采纳的建议；系统读取原简历片段和对应 JD 要求；AI 生成改写草稿；前端展示改写前后差异；学生确认、手动微调或放弃；确认后保存为新的简历版本 |
| 后置条件 | 系统保存 AI 改写任务、新简历版本、建议采纳状态和版本变更摘要 |
| 异常流程 | 如果建议需要真实补充经历而非表达改写，系统不生成虚构草稿，只给出学习或补充材料提醒 |
...
| 阶段 | 能力 |
| --- | --- |
| V1.1 | AI 岗位发现、分层岗位推荐组合、多岗位批量匹配、岗位收藏、投递记录、AI 简历改写、简历版本对比 |
...
| A03 | MVP 不依赖公开招聘网站爬虫，岗位主要来自用户粘贴、文件导入或后台维护；AI 自动发现岗位仅接入授权数据源或后台岗位库。 |
...
| AC08 | 分层岗位推荐 | 系统能基于候选岗位生成分层岗位推荐组合，并展示岗位匹配度、申请成功率预测、机会价值、风险等级和分层理由。 |
| AC09 | AI 简历改写 | 用户采纳建议后，系统能生成改写草稿、展示差异，并在用户确认后创建新简历版本。 |
...
| 自动发现并组合推荐岗位 | FR17、FR18 | 岗位发现服务、授权数据源适配器、机会梯度分层策略 |
| 用户确认后自动改写简历 | FR12、FR13、FR19 | AI 改写服务、差异预览、简历版本管理 |
